...
angular.module('app.forum', [])
	.directive('uploadForm', function() {
		return {
            ...
            transclude: true,
            replace: true,
            template: '<form method="post" class="dropzone" enctype="multipart/form-data" id="mydropzone">\
							<button type="submit" class="btn btn-primary btn-circle btn-lg pull-right"><i class="glyphicon glyphicon-cloud-upload"></i></button>\
						</form>',
			link: function(scope, element, attrs) {
				...
				var pagefunction = function() {
					...
					element.dropzone({
						url: "/ESVC/jsp/uploadFile.jsp?file-upload="+scope.currentpath,
						// url: "@Url.Action("User1")",
						addRemoveLinks : true,
						maxFilesize: 0.5, // 0.5 MB
						...
						init: function() {
							...
							this.element.querySelector("button[type=submit]").addEventListener("click",   function(e) {
					      	...
					      	this.on("successmultiple", function(file, responseText) {
							    ...
							    console.log('success', file, responseText.trim());
						    });
						    this.on("errormultiple", function(file, responseText) {
							    // Handle the responseText here. For example, add the text to the preview element:
							    // file.previewTemplate.appendChild(document.createTextNode(responseText));
							    console.log('fail');
						    });
			...
			link: function(scope, element, attrs) {
				...
				var pagefunction = function() {
					if(scope.content != null){
						...
						element.append(scope.content + scope.files);
					}
					// summernote
					element.summernote({
						height : 150,
					...
					function sendFile(file, editor, welEditable) {
						// console.log(scope.uploadpath, scope.foldername, file);
						// var path = scope.userinfo.U_Organization+scope.userinfo.U_Name+scope.userinfo.U_Password;
						// var localurl='/ESVC/jsp/uploadFile.jsp?file-upload=H:/syscom/凌群Demo/jetty_demo/webapps/root/ESVC/uploads/'+path+'/';
			            data = new FormData();
			            data.append("file", file);
			            $.ajax({
			                data: data,
			                type: "POST",
			                url: scope.uploadpath + scope.foldername,
			                cache: false,
			                contentType: false,
			                processData: false,
			                success: function(url) {
			                	console.log('success', url.trim());
				...
				loadScript("js/plugin/summernote/summernote.min.js", function(){
					loadScript("js/plugin/summernote/less.min.js", pagefunction);
				});
			}
		}
	})

	.directive('masterWidget', function() {
		return {
			restrict: 'E',
			scope: {
				cntype: "@cntype",
            ...
            templateUrl: 'views/MasterPageChild/MasterWidget.html',
			link: function(scope, element, attrs) {
				// bug: loading last object and call this, it will correct.
				if(scope.cntype == '其它類')
					pageSetUp();
			}
		}
	...
	.directive('faqTable', function() {
		return {
			restrict: 'AE',
            transclude: true,
            replace: true,
            controller: 'FAQCtrl',
            template: '<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%" ng-init="BestFAQ(readyToFAQ)">\
							<thead>\
								<tr>\
									<th data-hide="phone">No</th>\
									<th data-class="expand"><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 姓名</th>\
									<th data-hide="phone,tablet">類別</th>\
									<th data-hide="phone"><i class="fa fa-fw fa-phone text-muted hidden-md hidden-sm hidden-xs"></i> 主題</th>\
									<th>標題</th>\
									<th data-hide="phone,tablet"><i class="fa fa-fw fa-calendar txt-color-blue hidden-md hidden-sm hidden-xs"></i> 時間</th>\
								</tr>\
							</thead>\
							<tbody>\
								<tr data-ng-repeat="data in FAQ">\
									<td>{{$index+1}}</td>\
									<td>{{data[0].F_UserName}}</td>\
									<td>{{data[0].F_TypeName}}</td>\
									<td>{{data[0].F_TypeSubName}}</td>\
									<td><a href="javascript:void(0);">{{data[0].F_Topic}}</a></td>\
									<td>{{data[0].F_PublishDT}}</td>\
								</tr>\
							</tbody>\
						</table>',
			link: function(scope, element, attrs) {
				...
				scope.$watch("FAQ", function (newVal, oldVal){
					...
					if(newVal.length != oldVal.length){
						...
						// for (var i in newVal) {
						// 	for (var j in newVal[i]){
						// 		// console.log(newVal[i].index);
						// 		table.oApi._fnAddData(oSettings, [
						// 			parseInt(i)+1,
						// 			newVal[i][0].F_UserName,
						// 			newVal[i][0].F_TypeName,
						// 			newVal[i][0].F_TypeSubName,
						// 			newVal[i][0].F_Topic,
						// 			newVal[i][0].F_PublishDT]);
						// 		break;
						// 	}
						// }
